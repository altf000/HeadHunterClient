...
public class VacancyLocalStorage implements VacancyStore {
    ...
    @Override
    public Observable<VacancyList> getVacancies(String query) {
    ...
    public void clearVacancyList() {
        Realm realm = Realm.getDefaultInstance();
        realm.executeTransaction(realm1 -> realm1.delete(VacancyList.class));
        realm.close();
    }
    ...
    public void saveVacancyList(VacancyList vacancyList) {
        Realm realm = Realm.getDefaultInstance();
        realm.executeTransaction(realm1 -> realm1.insertOrUpdate(vacancyList));
        realm.close();
    }
